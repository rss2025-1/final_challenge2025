...
class SimpleLaneFollower(Node):
    ...
    def average_center_line(self, left_line, right_line):
        ...
        if x1_mid == x2_mid:
            ...
            return [x1_mid, y_top_screen, x1_mid, y_bottom_screen], [320, 240]

        slope = (x2_mid - x1_mid) / float(y2_mid - y1_mid) if (x2_mid - x1_mid) != 0 else 0.001
        intercept = y1_mid - slope * x1_mid
        x_at_top = int(round((x1_mid - intercept) / slope))
        x_at_bottom = int(round((y_bottom_screen - intercept) / slope))
        self.get_logger().info(f"top x: {x_at_top}")
        self.get_logger().info(f"bottom x: {x_at_bottom}")
        ...
        # slope = (y2_mid - y1_mid) / float(x2_mid - x1_mid) if (y2_mid - y1_mid) != 0 else 0.001
        # intercept = y1_mid - slope * x1_mid
...
####################################################################################################################################################################################
    ...
    def calculate_steering_angle_pp(self, x, y):
        ...
        L = self.wheelbase
        L1 = self.look_ahead
        steer_angle = np.arctan2(2*np.sin(eta)*L, L1)[0]
