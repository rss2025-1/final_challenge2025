...
class MotionController(Node):
    def __init__(self):
        ...
        self.start_pose_sub = self.create_subscription(PoseWithCovarianceStamped, '/initialpose', self.pose_callback, 1)

        self.goal_pose_pub = self.create_publisher(PoseStamped, '/goal_pose', 10)

        self.place_start_pose = True
    ...
    def pose_callback(self, msg: PoseWithCovarianceStamped):
        if self.place_start_pose:
            self.get_logger().info('Pose callback entered and place start pose is True.')
            self.start_pose_pub.publish(msg)
            self.place_start_pose = False
